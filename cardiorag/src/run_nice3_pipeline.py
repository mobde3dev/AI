"""
Pipeline orchestrator for the CardioRAG NICE3 (NG238) processing pipeline.
...
from src.clean_text import clean_all_pages
...
from src.chunk_nice3 import (
    NiceChunk,
    chunk_nice_section_block,
    reset_id_counters,
    count_tokens,
)
from src.deduplicate import deduplicate_nice_chunks
from src.validate_chunks import validate_nice3_chunks, ValidationReport
...
def run_nice3_pipeline(pdf_path: Optional[str] = None):
    """Run the complete NICE3 parsing and chunking pipeline."""
    start_time = time.time()

    if pdf_path is None:
        pdf_path = str(PROJECT_ROOT / "data" / "raw" / "NICE3.pdf")

    output_dir = str(PROJECT_ROOT / "data" / "processed")
    log_file = str(PROJECT_ROOT / "nice3_pipeline.log")

    setup_logging(log_file)
    logger = logging.getLogger(__name__)
    logger.info("Starting NICE3 (NG238) Pipeline on %s", pdf_path)
    ...
    # Output writing
    json_path = os.path.join(output_dir, "nice3_chunks.json")
    jsonl_path = os.path.join(output_dir, "nice3_chunks.jsonl")
    preview_path = os.path.join(output_dir, "nice3_chunks_preview.md")
    report_path = os.path.join(output_dir, "nice3_processing_report.json")
    ...
    print("\n" + "=" * 65)
    print("NICE3 PIPELINE EXECUTION COMPLETE")
    print("=" * 65)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="CardioRAG NICE3 Pipeline")
    parser.add_argument("--pdf", type=str, default=None, help="Path to NICE3.pdf")
    args = parser.parse_args()

    run_nice3_pipeline(pdf_path=args.pdf)
